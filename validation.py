...
class Train:
    ...
    def __init__(self):
        self.wtsInput2Hidden, self.wtsHidden2Output = self.train_network()
    ...
    def initialise(self):
        ...
        X = X.transpose()
        self.validationSet = X[(32000-3200):,:]
        self.validationResult = y[(32000-3200):]
        X = X[0: (32000-3200),:]
        y = y[0: (32000-3200)]
        # print(X.shape)
        # print(self.validationSet.shape)
        # input()
    ...
    def train_network(self):
        ...
        X, y, trainingLabel, self.feat_scal_avg, self.feat_scal_diff, self.feat_min, self.feat_std = self.initialise()
    ...
    #     print(outputs.shape)
        ...
        # Validation Starts
        testResultHidden, testResultFinal = self.forward_pass( self.validationSet, wtsInput2Hidden, wtsHidden2Output)
        testResult = np.argmax(testResultFinal, axis=1) + 1
        print(testResult.shape)
        print(testResult)
        correct = 0
        for label in (testResult - self.validationResult):
            if label == 0:
                correct = correct + 1
        print('accuracy on Validation Set is: ',(correct / testResult.size)*100,'%')
        # Validation ends
        return wtsInput2Hidden, wtsHidden2Output
        ...
        with open(out_file,'w') as f:
            ...
            with open(test_file, 'r') as tst:
                for line in tst:
                    ...
                    X = X - feat_min
                    X = X / feat_scal_diff
                    label = self.output_fun(X, wtsInput2Hidden, wtsHidden2Output )
                    print(i, label, file = f,sep=',')
                    i = i+1
                    testResult = np.append(testResult, label)
...
def main():
    ...
    tr =  Train()
    # testResult = Test().test_network( tr.feat_scal_avg, tr.feat_scal_diff, tr.feat_min, tr.feat_std, tr.wtsInput2Hidden, tr.wtsHidden2Output )
